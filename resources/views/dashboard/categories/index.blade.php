...
<div x-data="{
    showForm: false, editMode: false, deleteModal: false,
    ...
    openCreate() { this.editMode = false; this.editId = null; this.form = { name: '', slug: '' }; this.showForm = true; },
    openEdit(c) { this.editMode = true; this.editId = c.id; this.form = { name: c.name, slug: c.slug }; this.showForm = true; }
}">
                ...
                <div class="overflow-x-auto">
                    <table class="w-full">
                        ...
                        <tbody class="divide-y divide-gray-50">
                            @php $dotColors = ['bg-blue-500', 'bg-emerald-500', 'bg-purple-500', 'bg-amber-500', 'bg-rose-500', 'bg-cyan-500', 'bg-indigo-500', 'bg-teal-500']; @endphp
                            @forelse($categories as $category)
                                <tr class="hover:bg-gray-50/50 transition-colors group">
                                    <td class="px-6 py-4">
                                        <div class="flex items-center gap-3">
                                            <div class="w-3 h-3 rounded-full {{ $dotColors[$loop->index % count($dotColors)] }} flex-shrink-0"></div>
                                            <span class="text-sm font-semibold text-pens-navy">{{ $category->name }}</span>
                                        </div>
                                    </td>
                                    <td class="px-6 py-4 hidden sm:table-cell">
                                        <code class="text-xs bg-gray-100 text-gray-600 px-2 py-1 rounded-md">{{ $category->slug }}</code>
                                    </td>
                                    <td class="px-6 py-4 hidden md:table-cell">
                                        <span class="text-sm font-semibold text-pens-navy">{{ $category->posts_count }}</span>
                                        <span class="text-xs text-gray-400">berita</span>
                                    </td>
                                    <td class="px-6 py-4 text-sm text-gray-400 hidden lg:table-cell whitespace-nowrap">{{ $category->created_at->format('d M Y') }}</td>
                                    <td class="px-6 py-4">
                                        <div class="flex items-center justify-end gap-1">
                                            <button @click="openEdit({ id: {{ $category->id }}, name: '{{ addslashes($category->name) }}', slug: '{{ $category->slug }}' })" class="p-2 text-gray-400 hover:text-pens-cyan hover:bg-pens-cyan/10 rounded-lg transition-all" title="Edit">
                                                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"/></svg>
                                            </button>
                                            <button @click="deleteModal = true; deleteTarget = '{{ addslashes($category->name) }}'; deleteId = {{ $category->id }}" class="p-2 text-gray-400 hover:text-red-500 hover:bg-red-50 rounded-lg transition-all" title="Hapus">
                                ...
                                <tr>
                                    <td colspan="5" class="px-6 py-12 text-center">
                                        <div class="flex flex-col items-center gap-2">
                                            <svg class="w-10 h-10 text-gray-200" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M7 7h.01M7 3h5c.512 0 1.024.195 1.414.586l7 7a2 2 0 010 2.828l-7 7a2 2 0 01-2.828 0l-7-7A1.994 1.994 0 013 12V7a4 4 0 014-4z"/></svg>
                                            <p class="text-sm text-gray-400 font-medium">Belum ada kategori</p>
                                            <p class="text-xs text-gray-300">Klik "Tambah Kategori" untuk membuat kategori baru</p>
                                        </div>
                                    </td>
                                </tr>
                            @endforelse
            ...
            <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 sticky top-24" x-show="showForm" x-transition:enter="transition ease-out duration-200" x-transition:enter-start="opacity-0 translate-y-2" x-transition:enter-end="opacity-100 translate-y-0">
                ...
                <form :action="editMode ? '{{ url('categories') }}/' + editId : '{{ route('categories.store') }}'" method="POST" class="space-y-4">
                    ...
                    <div>
                        ...
                        <input type="text" name="name" x-model="form.name" @input="generateSlug()" class="w-full px-4 py-2.5 bg-gray-50 border border-gray-200 rounded-xl text-sm focus:ring-2 focus:ring-pens-cyan/30 focus:border-pens-cyan" placeholder="Nama kategori">
                    </div>
                    <div>
                        <label class="block text-xs font-semibold text-gray-500 uppercase tracking-wider mb-1.5">Slug</label>
                        <input type="text" name="slug" x-model="form.slug" class="w-full px-4 py-2.5 bg-gray-50 border border-gray-200 rounded-xl text-sm focus:ring-2 focus:ring-pens-cyan/30 focus:border-pens-cyan" placeholder="auto-generated">
                    </div>
                    <button type="submit" class="w-full px-4 py-2.5 text-sm font-semibold text-white bg-blue-600 rounded-xl hover:shadow-lg hover:bg-blue-700 transition-all" x-text="editMode ? 'Simpan Perubahan' : 'Tambah Kategori'"></button>
                </form>
            </div>
    ...
    <x-dashboard.modal id="deleteModal" title="Hapus Kategori">
        ...
        <x-slot name="footer">
            ...
            <form :action="'{{ url('categories') }}/' + deleteId" method="POST" class="inline">
                @csrf
                @method('DELETE')
